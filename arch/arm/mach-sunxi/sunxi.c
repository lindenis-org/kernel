...
int sunxi_get_mixinfo(char *cmdl, unsigned long *addr, unsigned long *len)
{
	char *endp;
	char *p = strstr(cmdl, "mix=");
	if (!p)
		return -EIO;

	*len = memparse(p + 4, &endp);
	if (*endp == '@')
		*addr = memparse(endp + 1, NULL);
	return 0;
}
EXPORT_SYMBOL(sunxi_get_mixinfo);

static void __init sunxi_reserve(void)
{
	unsigned long size;
	unsigned long start;

	if (sunxi_get_mixinfo(boot_command_line, &start, &size))
		return;
