...
static int sunxi_check_r1_ready_may_sleep(struct sunxi_mmc_host *smc_host)
{
	unsigned int cnt = 0;
	/*
	*SUNXI_MAX_R1B_TIMEOUT-10ms(dead wait)-(10)
	*(wait interval 10us,all wait 10*1000 us=10ms)**
	*/
	unsigned int delay_max_cnt[] = { 1000,
		SUNXI_MAX_R1B_TIMEOUT_MS - 10 - 10 };
	int i = 0;
...
/******************************************************/
	...
	if (mrq_retry) {
		...
		cmd = mrq_retry->sbc?mrq_retry->sbc:mrq_retry->cmd;
...
static int sunxi_mmc_extra_of_parse(struct mmc_host *host)
{
	...
	if (of_property_read_bool(np, "cap-wait-while-busy"))
