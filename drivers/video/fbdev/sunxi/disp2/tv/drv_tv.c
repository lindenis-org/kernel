...
void tv_report_hpd_work(u32 sel, u32 hpd)
{
	...
	switch (hpd) {
	case STATUE_CLOSE:
		extcon_set_state(&extcon_dev[sel], STATUE_CLOSE);
		if (is_compatible_cvbs)
			extcon_set_state(&extcon_cvbs, STATUE_CLOSE);
		break;

	case STATUE_OPEN:
		extcon_set_state(&extcon_dev[sel], STATUE_OPEN);
		if (is_compatible_cvbs)
			extcon_set_state(&extcon_cvbs, STATUE_OPEN);
		break;
...
s32 tv_init(struct platform_device *pdev)
{
	...
	if (ret < 0)
		...
		if (interface == DISP_TV_CVBS) {
			snprintf(extcon_name, sizeof(extcon_name),
				"tve%d_cvbs", sel);
			...
			if (sel == 0) {
				extcon_cvbs.supported_cable = tv_cable;
				extcon_dev_register_attr(&extcon_cvbs,
								&pdev->dev);
		...
		extcon_dev[sel].supported_cable = tv_cable;
		extcon_dev_register_attr(&extcon_dev[sel], &pdev->dev);
