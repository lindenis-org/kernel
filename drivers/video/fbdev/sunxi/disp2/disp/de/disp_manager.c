...
static s32 disp_lyr_dump(struct disp_layer *lyr, char *buf)
{
	...
	count += sprintf(buf + count, "fb[%4d,%4d;%4d,%4d;%4d,%4d] ",
			 ...
			 data.config.info.fb.size[0].width,
			 data.config.info.fb.size[0].height);
	count += sprintf(buf + count, "crop[%4d,%4d,%4d,%4d] ",
...
static s32 disp_mgr_dump(struct disp_manager *mgr, char *buf)
{
	...
	count += sprintf(
	    buf + count,
	    "mgr%d: %dx%d fmt[%s] cs[0x%x] range[%s] %s direct_show[%s] "
	    "\n",
	    mgr->disp, mgrp->cfg->config.size.width,
