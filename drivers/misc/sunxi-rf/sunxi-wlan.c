	...
	if (on_off) {
		if ((wlan_data->power_state) == 0) {
			...
			if (gpio_is_valid(wlan_data->gpio_wlan_en))
				gpio_direction_output(wlan_data->gpio_wlan_en, !wlan_data->gpio_wlan_en_active);
		}
	} else {
		if ((wlan_data->power_state) == 0) {
			/* set wifi_en off */
...
static int sunxi_wlan_on(struct sunxi_wlan_platdata *data, bool on_off)
{
	...
	if (!on_off && gpio_is_valid(data->gpio_wlan_en)) {
		if (!bt_power_state)
			gpio_direction_output(data->gpio_wlan_en, data->gpio_wlan_en_active);
	}
...
static int sunxi_wlan_probe(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(data->gpio_wlan_en)) {
		...
		data->gpio_wlan_en_active = config.data;
		...
		ret = gpio_direction_output(data->gpio_wlan_en, data->gpio_wlan_en_active);
		if (ret < 0) {
