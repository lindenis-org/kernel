...
ssize_t fsg_show_zero_disk(struct fsg_lun *curlun, char *buf)
{
	return sprintf(buf, "%u\n", curlun->zero_disk);
}
EXPORT_SYMBOL_GPL(fsg_show_zero_disk);

ssize_t fsg_store_zero_disk(struct fsg_lun *curlun,
			const char *buf, size_t count)
{
	int value = 0;
	int ret = 0;
...
EXPORT_SYMBOL_GPL(fsg_store_zero_disk);
#endif
