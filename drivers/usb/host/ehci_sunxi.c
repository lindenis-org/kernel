...
extern struct usb_device *glhub_udev;
		...
		const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "test_not_operating", 18)) {
		...
		DMSG_INFO("test_mode:0x%x\n", test_mode);
	} else if (!strncmp(buf, "hub_test_pack_port", 18)) {
		port_num = (u8)simple_strtoull((buf + 18), NULL, 10);
		DMSG_PANIC("testing port_num = %d \n", port_num);

		if (port_num > 0 && port_num < 8 && glhub_udev) {
			ret = usb_control_msg(glhub_udev,
				usb_sndctrlpipe(glhub_udev, 0),
				USB_REQ_SET_FEATURE,
				USB_RT_PORT,
				USB_PORT_FEAT_TEST,
				(USB_PORT_TEST_PACKET<<8)|(port_num & 0xff),
				NULL,
				0,
				3000);
			DMSG_PANIC("ret is %d\n", ret);
			if (ret < 0)
				DMSG_PANIC("ERR: hub_test_pack_port, ret is %d\n", ret);
		} else {
			DMSG_PANIC("ERR: hub is %p, port num is %d, port num must be 1 to 7!\n",
					glhub_udev, port_num);
		}
...
int sunxi_insmod_ehci(struct platform_device *pdev)
{
	...
	if (sunxi_ehci->wakeup_suspend)
		scene_lock_init(&ehci_standby_lock[sunxi_ehci->usbc_no],
				SCENE_USB_STANDBY,  "ehci_standby");
#endif
