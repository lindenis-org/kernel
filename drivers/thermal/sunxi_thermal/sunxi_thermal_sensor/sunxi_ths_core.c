...
static inline u32 sunxi_ths_readl(void __iomem *addr)
{
...
static inline void sunxi_ths_writel(u32 val, void __iomem *addr)
{
#if  defined(CONFIG_SUNXI_TRUSTZONE)
...
int ths_driver_get_temp(struct sunxi_ths_data *ths_data, int id)
{
	...
	reg_data = sunxi_ths_readl(ths_data->base_addr + status_reg->offset);
	...
	reg_data = sunxi_ths_readl(ths_data->base_addr
	...
	/* clear the status */
	sunxi_ths_writel(reg_flag, ths_data->base_addr + status_reg->offset);
	mutex_unlock(&ths_data->ths_mutex_lock);
	...
	for (i = 0, calcu_times = 0; i < ths_data->sensor_cnt; i++) {
		...
		if (calcu_times == 0) {
			...
			if ((i + 1) < ths_data->sensor_cnt) {
				...
				write_value = sensor_temp;
				sunxi_ths_writel(write_value, ths_data->base_addr +
						reg->offset + (i / 2) * 4);
				thsprintk("sensor:%d efuse_reg[%d] =0x%x\n", i, i / 2, write_value);
				break;
			}
					  ...
					  u32 *ths_cal_data)
{
	...
	for (i = 0; i < cal_reg_num; i++) {
		...
		thsprintk("ths_cal_data[%d] =0x%x\n",
						i, *(ths_cal_data + i));
		sunxi_ths_writel(*(ths_cal_data + i), ths_data->base_addr
						      + reg->offset + i * 4);
		reg_data = sunxi_ths_readl(ths_data->base_addr
					   + reg->offset + i * 4);
					  ...
					  struct thermal_reg *reg)
{
	...
	ret = sunxi_efuse_read(EFUSE_THM_SENSOR_NAME, (void *)(ths_cal_data));
	if (ret) {
					...
					struct thermal_reg *reg)
{
	...
	ths_drv_ver = get_ths_driver_version(ths_data);
	reg_value = sunxi_ths_readl(ths_data->base_addr + reg->offset);
	if (ths_drv_ver == 1) {
	...
	sunxi_ths_writel(reg_value, ths_data->base_addr + reg->offset);
}
			  ...
			  struct thermal_sensor_coefficent *ths_coefficent)
{
	...
	while (strcmp("", thermal_reg->name)) {
		thsprintk("reg_name=%s, value=0x%x\n",
				thermal_reg->name,
				sunxi_ths_readl(ths_data->base_addr +
						thermal_reg->offset));
		thermal_reg++;
	}
}
					...
					struct temp_calculate_coefficent *cpara)
{
	...
	for (i = 0; i < ths_data->sensor_cnt; i++) {
		...
		sunxi_ths_writel(reg_value, ths_data->base_addr + reg->offset + i * 4);
	}
}
		     ...
		     struct thermal_sensor_coefficent *ths_coefficent)
{
	...
	while (strcmp("", thermal_reg->name)) {
		...
		case NORMAL_REG:
			sunxi_ths_writel((thermal_reg->value),
				ths_data->base_addr + (thermal_reg->offset));
			break;

		case ENABLE_REG:
			if ((ths_drv_ver == 1) || (ths_drv_ver == 2))
				sunxi_ths_writel(thermal_reg->value, ths_data->base_addr + thermal_reg->offset);
		...
		thsprintk("reg_name=%s, offset=0x%x, value=%x, reg_value=%x\n",
			...
			thermal_reg->offset,
			thermal_reg->value,
			sunxi_ths_readl(ths_data->base_addr + (thermal_reg->offset)));
		thermal_reg++;
	}
...
static void ths_driver_disable_old_new_enreg(struct sunxi_ths_data *ths_data,
			      struct thermal_reg *reg)
{
	sunxi_ths_writel(DISABLE_EN_REG, ths_data->base_addr + reg->offset);
}

static void ths_driver_disable_chop_reg(struct sunxi_ths_data *ths_data)
{
	sunxi_ths_writel(0, ths_data->base_addr + 0x04);
}
