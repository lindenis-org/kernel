...
int arisc_dvfs_cfg_vf_table(void *vf, unsigned int cpu,
				unsigned vf_size, unsigned vf_num)
{
	int index = 0;
	...
	for (index = 0; index < vf_num; index++) {
		...
		pmessage->paras[1]   = ((struct cpufreq_dvfs_table *)(vf) + index)->freq;
		pmessage->paras[2]   = ((struct cpufreq_dvfs_table *)(vf) + index)->voltage / 1000;
		pmessage->paras[3]   = ((struct cpufreq_dvfs_table *)(vf) + index)->axi_div;
