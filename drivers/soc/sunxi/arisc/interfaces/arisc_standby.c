			...
			struct sunxi_idle_para *para)
{
	...
	pmessage->paras[0]      = para->cpu;
	pmessage->paras[1]      = para->level;
	pmessage->paras[2]      = (unsigned int)para->resume_addr;
...
int arisc_standby_super(struct super_standby_para *para, arisc_cb_t cb, void *cb_arg)
{
	...
	/* initialize message */
	if (para->pextended_standby == (u32)NULL) {
	...
	printk("send suspend message to arisc\n");
...
int arisc_sysconfig_ir_paras(void)
{
	...
	if (!np) {
		ARISC_ERR("No allwinner,s_cir node found\n");
		return -ENODEV;
	}

	if (of_property_read_u32(np, "ir_power_key_code", &ir_power_key_code)) {
		ARISC_ERR("parse ir power key code fail\n");
		return -EINVAL;
	}
