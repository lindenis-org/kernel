...
typedef struct sbrom_toc0_config {
	...
	unsigned int      	dram_para[32];  	/* dram 参数 */
	int			uart_port;      	/* UART 控制器编号 */
	normal_gpio_cfg   	uart_ctrl[2];    	/* UART 控制器GPIO */
	int              	enable_jtag;    	/* JTAG使能 */
	normal_gpio_cfg   	jtag_gpio[5];    	/* JTAG 控制器GPIO */
	normal_gpio_cfg  	storage_gpio[50]; 	/* 存储设备 GPIO信息 */
	/* 0-23 放nand，24-31存放卡0，32-39放卡2 */
	/* 40-49 存放spi */
