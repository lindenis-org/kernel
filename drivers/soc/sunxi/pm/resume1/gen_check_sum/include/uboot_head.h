...
struct spare_boot_data_head {
	...
	normal_gpio_cfg	uart_gpio[2];	/* UART 控制器(调试打印口)GPIO信息 */
	int	twi_port;	/* TWI 控制器编号 */
	normal_gpio_cfg	twi_gpio[2];	/* TWI 控制器GPIO信息，用于控制TWI */
	int	work_mode;	/* 工作模式 */
	int	storage_type;	/* 存储介质类型  0：nand   1：sdcard	2: spinor */
	normal_gpio_cfg	nand_gpio[32];	/* nand GPIO 信息 */
	char	nand_spare_data[256];	/* nand  额外信息 */
	normal_gpio_cfg	sdcard_gpio[32];	/* sdcard GPIO 信息 */
