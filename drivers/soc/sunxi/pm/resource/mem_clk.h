/*
 * drivers/soc/sunxi/pm/resource/mem_clk.h
...
int mem_clk_save(void);
int mem_clk_restore(void);
#else
#if defined CONFIG_ARCH_SUN8IW11P1
#include "../platform/sun8iw11p1/pm_config_ccu_sun8iw11p1.h"
#else
...
struct ccm_state {
	struct __ccmu_reg_list_t *ccm_reg;
	struct __ccmu_reg_list_t ccm_reg_backup;
};
...
struct clk_state {
	struct __ccmu_reg_list_t *CmuReg;
...
struct __ccmu_reg_list_t *mem_clk_init(__u32 mmu_flag);
struct __ccmu_reg_list_t *mem_get_ba(void);
