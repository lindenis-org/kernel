...
#include "../soc/sunxi/arisc/include/arisc_includes.h"
					...
					unsigned int index)
{
	...
	freq = policy->freq_table[index].frequency * 1000;
				...
				unsigned int cpu)
{
	...
	cpufreq_for_each_valid_entry(pos, table) {
		...
		cpufreq_dvfs_table[num].voltage = dev_pm_opp_get_voltage(opp);
		cpufreq_dvfs_table[num].freq = dev_pm_opp_get_freq(opp);
		cpufreq_dvfs_table[num].axi_div = dev_pm_opp_axi_bus_divide_ratio(opp);
		rcu_read_unlock();
		CPUFREQ_DBG("num:%d, volatge:%d, freq:%d, axi_div:%d ,%s\n", num, cpufreq_dvfs_table[num].voltage, cpufreq_dvfs_table[num].freq,
			cpufreq_dvfs_table[num].axi_div, __func__);
		num++;
	}

	ret = arisc_dvfs_cfg_vf_table(cpufreq_dvfs_table, cpu, sizeof(struct cpufreq_dvfs_table), num);
	if (ret != 0) {
...
static int sunxi_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	if (of_property_read_u32(dvfs_main_np, "opp_table_count", &table_count)) {
	...
	ret = dev_pm_opp_of_get_sharing_cpus_by_soc_bin(cpu_dev, policy->cpus, soc_bin);
	if (ret) {
		CPUFREQ_ERR("OPP-v2 opp-shared Error\n");
		goto out_err_clk_pll;
	}

	ret = dev_pm_opp_of_cpumask_add_table_by_soc_bin(policy->cpus, soc_bin);
	if (ret) {
