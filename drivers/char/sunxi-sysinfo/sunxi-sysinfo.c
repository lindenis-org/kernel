...
struct sunxi_soc_info {
	void __iomem *mixaddr;
	unsigned long mixlen;
	s8 key_name[0];
};

struct sunxi_soc_info *sinfo;

static int soc_info_open(struct inode *inode, struct file *file)
{
	file->private_data = sinfo;
	return 0;
}
...
static long soc_info_ioctl(struct file *file, unsigned int cmd,
		unsigned long ioctl_param)
{
	struct sunxi_soc_info *sf = file->private_data;
	int ret = 0;
	char id[16] = "";
	...
	case CHECK_SOC_BONDING:
		...
		pr_debug("soc id:%s\n", id);
		break;
	case CHECK_SOC_MIXADDR:
		ret = copy_to_user((void __user *)ioctl_param, (char *)&sf->mixlen, sizeof(unsigned long));
		break;
	default:
...
static int soc_info_mmap(struct file *filp, struct vm_area_struct *vma)
{
	struct sunxi_soc_info *sf = filp->private_data;
	unsigned long mixlen = vma->vm_end - vma->vm_start;
	...
	if (remap_pfn_range(vma, vma->vm_start,
			__phys_to_pfn((u32)sf->mixaddr),
			sf->mixlen, vma->vm_page_prot)) {
			     ...
			     struct class_attribute *attr, char *buf)
{
	...
	sunxi_efuse_readn(sinfo->key_name, key_data, 256);
	for (i = 0; i < 256; i++) {
		if ((i > 0) && (key_data[i] == 0))
			break;
		if ((i > 0) && (i % 8 == 0))
		...
		const char *buf, size_t count)
{
	...
	memset(sinfo->key_name, 0, SUNXI_KEY_NAME_LEN);
	strncpy(sinfo->key_name, buf, count);
	return count;
}
...
extern int sunxi_get_mixinfo(char *cmdl, unsigned long *addr, unsigned long *len);
static int __init sunxi_sys_info_init(void)
{
	s32 ret = 0;

	sinfo = kmalloc(sizeof(struct sunxi_soc_info) + SUNXI_KEY_NAME_LEN, GFP_KERNEL);

	if (IS_ERR_OR_NULL(sinfo))
		return -ENOMEM;

	if (sunxi_get_mixinfo(saved_command_line, (unsigned long *)&sinfo->mixaddr,
				&sinfo->mixlen)) {
		sinfo->mixaddr = NULL;
		sinfo->mixlen = 0;
	}
