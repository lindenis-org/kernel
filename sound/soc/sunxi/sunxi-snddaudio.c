		...
		struct snd_pcm_hw_params *params)
{
...
#if 1
	//AC108 SYSCLK select 24.576M MCLK
	ret = snd_soc_dai_set_sysclk(codec_dai, 0, freq, 0);
	if (ret < 0)
		dev_warn(card->dev, "codec_dai set sysclk failed\n");
#else
	//AC108 SYSCLK select PLL and PLLCLK source select BCLK
	ret = snd_soc_dai_set_sysclk(codec_dai, 1 , freq, 0);
	if (ret < 0) {
		dev_warn(card->dev, "the codec_dai set sysclk failed\n");
		return ret;
	}

	/*set pll*/
	ret = snd_soc_dai_set_pll(codec_dai, 1, 0, freq, freq);	// slot width
	if (ret < 0) {
		dev_warn(card->dev, "the codec_dai set pll failed\n");
		return ret;
	}
#endif
	...
	ret = snd_soc_dai_set_fmt(codec_dai, SND_SOC_DAIFMT_I2S | SND_SOC_DAIFMT_NB_IF | SND_SOC_DAIFMT_CBS_CFS);
#else
