...
EXPORT_SYMBOL_GPL(codec_digitaladress);
EXPORT_SYMBOL_GPL(codec_analogadress);
...
static int codec_init(struct sunxi_codec *sunxi_internal_codec)
{
	...
	snd_soc_update_bits(
	    sunxi_internal_codec->codec, SPKOUT_CTRL0, (0x1f << SPKOUT_VOL),
	    (sunxi_internal_codec->gain_config.spkervol << SPKOUT_VOL));
	snd_soc_update_bits(
	    sunxi_internal_codec->codec, HP_CTRL, (0x3f << HPVOL),
	    (sunxi_internal_codec->gain_config.headphonevol << HPVOL));
#endif
	snd_soc_update_bits(
	    sunxi_internal_codec->codec, MIC1_CTRL, (0x7 << MIC1BOOST),
		...
		struct snd_kcontrol *k, int event)
{
	...
	case	SND_SOC_DAPM_POST_PMU:
		if (spk_gpio.cfg) {
			gpio_set_value(spk_gpio.cfg, 1);
			mdelay(50);
		}
		snd_soc_update_bits(codec, SPKOUT_CTRL0,
	...
	case	SND_SOC_DAPM_PRE_PMD:
		if (spk_gpio.cfg) {
			gpio_set_value(spk_gpio.cfg, 1/*0*/);
			mdelay(50);
		}
		snd_soc_update_bits(codec, SPKOUT_CTRL0,
		...
		struct snd_kcontrol *k, int event)
{
	...
	case	SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_update_bits(codec, SPKOUT_CTRL1,
			...
			struct snd_kcontrol *k,	int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		/*open*/
		snd_soc_update_bits(codec, HP_PA_CTRL, (0xf << HPOUTPUTENABLE),
				    (0xf << HPOUTPUTENABLE));
		snd_soc_update_bits(codec, HP_CTRL, (0x1 << HPPA_EN),
				    (0x1 << HPPA_EN));
		msleep(9);
		snd_soc_update_bits(codec, MIX_DAC_CTRL, (0x3 << LHPPAMUTE),
				    (0x3 << LHPPAMUTE));
		break;
	case SND_SOC_DAPM_PRE_PMD:
		/*close*/
			...
			struct snd_kcontrol *k,	int event)
{
	...
	pr_debug("..lineout power state change.\n");
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		if (spk_gpio.cfg) {
			msleep(50);
		      ...
		      struct snd_kcontrol *kcontrol, int event)
{
	...
	pr_debug("%s,line:%d,SUNXI_ADC_DIG_CTRL(300):%x\n", __func__, __LINE__,
		 snd_soc_read(codec, SUNXI_ADC_DIG_CTRL));
...
static const struct soc_enum ac_lineout_func_enum =
	SOC_ENUM_SINGLE(LINEOUT_CTRL0, LINEOUTR_SRC, 2,
						ac_lineout_func_sel);
...
#endif
	/*0x284*/
	SND_SOC_DAPM_MUX("AIF2OUTL Mux", SUNXI_AIF2_ADCDAT_CTRL, 15, 0,
								&aif2outl_mux),
	SND_SOC_DAPM_MUX("AIF2OUTR Mux", SUNXI_AIF2_ADCDAT_CTRL, 14, 0,
								&aif2outr_mux),
	/*0x288*/
	SND_SOC_DAPM_MUX("AIF2INL Mux", SUNXI_AIF2_DACDAT_CTRL, 15, 0,
								&aif2inl_mux),
	SND_SOC_DAPM_MUX("AIF2INR Mux", SUNXI_AIF2_DACDAT_CTRL, 14, 0,
								&aif2inr_mux),
		...
		char *buf)
{
	...
	while (reg_labels[i].name != NULL) {
		if (reg_labels[i].value == AREG_MIN_NUM)
			reg_group++;
		if (reg_group != 1) {
...
static void sunxi_internal_codec_shutdown(struct platform_device *pdev)
{
	...
	/*snd_soc_update_bits(sunxi_internal_codec->codec,
		HP_CTRL, (0x1<<HPPA_EN), (0x0<<HPPA_EN));*/
#if 0
	snd_soc_update_bits(sunxi_internal_codec->codec, MIX_DAC_CTRL,
